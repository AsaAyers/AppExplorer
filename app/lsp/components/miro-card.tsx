...
type Props<M extends Meta> = Pick<
  CardProps,
  "height" | "width" | "x" | "y" | "style" | "fields" | "title"
> &
...
export const MiroCard = <M extends Meta>({
  ...
  return (
    <div className="app-card miro-draggable">
      <h1 className="app-card--title">{editTitle ?? title}</h1>
      <h1 className="app-card--description p-medium">
        {editDescription ?? description}
      </h1>
      <span style={{ display: "none" }} ref={self}>
        {description}
      </span>
      <div className="app-card--body">
        <div className="app-card--tags">
          <Tag>{meta?.projectName}</Tag>
          {fields?.map((field, i) => (
            <Tag key={i}>{field.value}</Tag>
          ))}
        </div>
      </div>
    </div>
