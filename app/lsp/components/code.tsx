...
export const Code = ({
  ...
  const annotatedLines = React.useMemo(() => {
    return lines.map((line, i) => {
      ...
      return annotationsForLine.reduce<Array<string | JSX.Element>>(
        (lineArray, annotation) => {
          const segment = lineArray[lineArray.length - 1];
          invariant(
            typeof segment === "string",
            "Expected segment to be a string"
          );
          const processedCharacters = line.length - segment.length;

          let start =
            (annotation.startLine === i ? annotation.startCharacter : 0) -
            processedCharacters;
          let end =
            (annotation.endLine === i ? annotation.endCharacter : line.length) -
            processedCharacters;
        ...
        {annotatedLines.map((line, i) => (
          <React.Fragment key={i}>
            ...
            {peek && peek === i && children && (
              <div className="peek">{children}</div>
            )}
          </React.Fragment>
        ))}
      </code>
